...
from Project4.read_data_p4 import read_cancer, read_glass

EPS = 1e-6
np.random.seed(0)


def weights_network(inputs_size, num_hidden_layers, num_hidden_nodes: [], outputs_size):
...
def forward_propagate(weight_network, row: np.ndarray):
    ...
    inputs = row
    output_network = []
    output_layer_index = len(weight_network) - 1
    for layers_idx in range(len(weight_network)):
        ...
        for hidden_layer, weights in weight_network[layers_idx].items():
            ...
            e_y = []
            # Applying soft-max function for output layer
            if layers_idx == output_layer_index:
                for i in y[0]:
                    e_y.append(np.exp(i))
                denominator = sum(e_y)
                for k in e_y:
                    output.append(k / denominator)
            # Applying sigmoid function to hidden layers
            else:
                for i in y[0]:
...
def backpropagate_errors(weight_network, output_network, row: np.ndarray):
    ...
    cls = row[:, -1][0]
    # output layer nodes in the network
    output_nodes = list(output_network[-1].values())[0]
    # number of output nodes is equal to the number of classes in the dataset
    num_cls = len(output_nodes)
    # Initializing the target list with zeros, and replacing the index with class number with 1
    target = [0] * num_cls
    target[int(cls)] = 1
    ...
    delta = []
    # Constructing output layer deltas where the outputs are calculated using soft-max function
    output_nodes_delta = []
    for i in range(num_cls):
        s_i = output_nodes[i][0]
        t_i = target[i]
        output_nodes_delta.append((t_i / s_i) * s_i * (1.0 - s_i))  # cross entropy error
...
def train_network(weight_network, dataset, learning_rate, epoc_num):
    """Training network for a fixed number of epochs"""
    # number of output nodes is equal to the number of classes stored in the last column of each dataset
    n_output_nodes = len(np.unique(dataset[:, -1], return_counts=False))
    row_num, _ = dataset.shape

    row_errors = []
    epoch_errors = []
    for epoch in range(epoc_num):
        epoch_error = 0
        prediction = []
        for row in dataset:
            ...
            output_nodes = list(output_network[-1].values())[0].T
            prediction.append(np.argmax(output_nodes))
            target_cls = int(row[0, -1])
            # the target output is a list of values all zeros except for the element with the index of the class number
            target_output = np.zeros(shape=(1, n_output_nodes))
            target_output[0, target_cls] = 1
            # compute error (cross entropy used instead of sum of squared error)
            row_error = np.sum((-target_output * np.log(output_nodes)))
...
def main():
    ...
    cancer_data_rows, cancer_data_features = cancer_data.shape
    glass_data_rows, glass_data_features = glass_data.shape

    outputs_size_cancer = len(np.unique(cancer_data[:, -1], return_counts=False))
    outputs_size_glass = len(np.unique(glass_data[:, -1], return_counts=False))

    network_weight_cancer = weights_network(inputs_size=(cancer_data_features - 1), num_hidden_layers=1,
                                            num_hidden_nodes=[9], outputs_size=1)
    network_weight_glass = weights_network(inputs_size=(glass_data_features - 1), num_hidden_layers=1,
                                           num_hidden_nodes=[9], outputs_size=outputs_size_glass)

    train_network(network_weight_cancer, cancer_data, 0.1, 3)
